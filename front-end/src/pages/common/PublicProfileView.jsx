...
const PublicProfileView = () => {
    ...
    const fetchProfileAndReviews = async () => {
      try {
        ...
        if (data.role === "TECHNICIAN" || data.role === "TECH") {
          try {
            // Pass technician ID to function
            const reviewData = await getTechnicianReviews(data.id);
            setReviews(reviewData || []);
          } catch (revErr) {
            console.error("Error fetching tech reviews:", revErr);
            // Gracefully handle error - show empty reviews
            setReviews([]);
          }
        }
        setLoading(false);
  ...
  const hasListings =
    isOwner && profile.activeListings && profile.activeListings.length > 0;
          ...
          >
            ...
            <div className="lg:col-span-8 xl:col-span-9 space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <StatCard
                  label={
                    isTech ? "Rating" : isOwner ? "Active Listings" : "Status"
                  }
                  value={
                    isTech
                      ? displayRating
                        ...
                        >
                          <div className="flex justify-between items-start mb-2">
                            <div className="flex items-center gap-3">
                              <img
                                src={
                                  rev.ownerProfileImageUrl
                                    ? rev.ownerProfileImageUrl.startsWith(
                                        "http",
                                      )
                                      ? rev.ownerProfileImageUrl
                                      : `http://localhost:8086/uploads/${rev.ownerProfileImageUrl}`
                                    : `https://ui-avatars.com/api/?name=${encodeURIComponent(rev.ownerName)}&background=random`
                                }
                                className="w-10 h-10 rounded-full object-cover border border-gray-200"
                                alt={rev.ownerName}
                                onError={(e) => {
                                  e.target.src = `https://ui-avatars.com/api/?name=${rev.ownerName}`;
                                }}
                              />
                              <div>
