...
const TechnicianProfile = () => {
  ...
  const getDisplayValue = (keyA, keyB, fallback) => {
    if (technician[keyA] !== undefined && technician[keyA] !== null)
      return technician[keyA];
    if (technician[keyB] !== undefined && technician[keyB] !== null)
      return technician[keyB];
    return fallback;
  };
  ...
  const displayJobs = getDisplayValue(
    "totalJobsCompleted",
    "technicianTotalJobs",
    0,
  );
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 text-center relative overflow-hidden">
            ...
            <div className="mt-2 flex flex-wrap justify-center gap-2">
              {technician?.skills && technician.skills.length > 0 ? (
                ...
                <span className="text-xs text-gray-400 italic font-medium tracking-tight uppercase">
                  No skills listed
                </span>
              )}
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <ul className="space-y-3 text-sm text-gray-600">
              ...
              <li className="flex items-center gap-3">
                ...
                <span className="font-bold">LKR {technician?.basePrice}</span> /
                visit
              </li>
            </ul>
          </div>
            ...
            {reviews.length > 0 ? (
              <div className="space-y-4">
                {reviews.map((r) => (
                  <div
                    key={r.id}
                    className="border-b pb-2 mb-2 last:border-0 last:pb-0"
                  >
                    <div className="flex justify-between items-start mb-1">
                      <div className="flex items-center gap-2">
                        ...
                        <img
                          src={
                            r.ownerProfileImageUrl
                              ? r.ownerProfileImageUrl.startsWith("http")
                                ? r.ownerProfileImageUrl
                                : `http://localhost:8086/uploads/${r.ownerProfileImageUrl}`
                              : `https://ui-avatars.com/api/?name=${encodeURIComponent(r.ownerName)}&background=random`
                          }
                          alt={r.ownerName}
                          className="w-8 h-8 rounded-full object-cover border border-gray-100"
                          onError={(e) => {
                            e.target.src = `https://ui-avatars.com/api/?name=${r.ownerName}`;
                          }}
                        />
